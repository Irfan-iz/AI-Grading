...
AI_VECT = None
SBERT_MODEL = None
# New globals for speed optimization
PLAGIARISM_DB = []
DB_EMBEDDINGS = None
...
def load_models():
    ...
    # 3. Load & Pre-Calculate Plagiarism Database (Speed Fix)
    if not PLAGIARISM_DB:
        ...
        # Process Content
        if not raw_text:
            print("Warning: plagiarism_data.txt not found or empty. Using fallback.")
            PLAGIARISM_DB = [
        ...
        # Calculate Vectors ONCE here (this fixes the lag!)
...
def check_plagiarism(text):
    """Checks for plagiarism against the pre-loaded database."""
    ...
    try:
        ...
        # Calculate similarity against PRE-CALCULATED embeddings (Instant)
        ...
        max_score = cosine_scores[0][max_score_idx].item()
        best_match_source = PLAGIARISM_DB[max_score_idx]
